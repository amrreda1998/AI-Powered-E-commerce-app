...
app.get('/api/products', async (req, res) => {
  try {
    ...
    const products = queryResults.matches?.map(match => ({
      id: match.id,
...
const initializeEmbedder = async () => {
  if (!embedder) {
    console.log('🤖 Loading all-MiniLM-L6-v2 model for API (reliable & tested)...');
    try {
      embedder = await pipeline('feature-extraction', 'Xenova/all-MiniLM-L6-v2');
      console.log('✅ API all-MiniLM-L6-v2 model loaded successfully!');
    } catch (error) {
      console.error('❌ Failed to load API MiniLM-L6 transformer model:', error);
      throw error;
    }
  }
  return embedder;
...
// Generate semantic embeddings using transformers.js (same as seed.ts)
